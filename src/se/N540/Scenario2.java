...
public class Scenario2 {
	...
	private static final Integer NUMBER_OF_VM = 400;
	private static final Integer NUMBER_OF_TASKS = 1500;
	...
	private static List<Cloudlet> createTasks(Integer numberOfTasks,
		...
		for (Integer i = 0; i < numberOfTasks; i++) {
			Integer taskDuration = rand.nextInt((maxTime - minTime) + 1) + minTime;
			Integer numberOfCpu = rand.nextInt((maxCpu - minCpu) + 1) + minCpu;
	...
	private static void printCloudletList(Calendar cal, List<Cloudlet> list) {
		...
		Map<Integer, Integer> cpuReq = new HashMap<>();
		...
		try {
			...
			sb.append("Cloudlet ID").append(indent).append("STATUS").append(indent)
				...
				.append("Finish time").append(indent).append("Required Time").append(indent)
				.append("Resources").append("\n");
			
			for (Integer i = 0; i < size; i++) {
				cloudlet = list.get(i);
				Integer cloudletCpu = cloudlet.getNumberOfPes();
				Integer cpuCount = cpuReq.get(cloudletCpu);
				if (cpuCount != null) {
					cpuReq.put(cloudletCpu, ++cpuCount);
				} else {
					cpuReq.put(cloudletCpu, 1);
				}
				...
				sb.append(indent).append(indent).append(cloudlet.getResourceId())
					...
					.append(indent).append(indent).append(dft.format(cloudlet.getCloudletLength()))
			...
			sb.append(collectCpuCount(cpuReq));
			sb.append("\n");
		...
		for (int i = 0; i < size; i++) {
			...
			Log.printLine(indent + indent + cloudlet.getResourceId() + indent
					...
					+ dft.format(cloudlet.getCloudletLength()) + indent + indent
	...
	private static String collectCpuCount(Map<Integer, Integer> cpuCount) {
		StringBuilder sb = new StringBuilder();
		for (Map.Entry<Integer, Integer> entry: cpuCount.entrySet()) {
			sb.append("Number of CPUs: ").append(entry.getKey()).append(" Usage: ").append(entry.getValue());
			sb.append("\n");
		}
	...
	private static BufferedWriter createOutput(Calendar cal) {
		...
		String outputFile = Paths.get("results", "scenario2biii_" + sdf.format(cal.getTime())).toString();
